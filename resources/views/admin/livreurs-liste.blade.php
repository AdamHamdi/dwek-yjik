...
<div class="row mt-5">
    <div class="col-md-12">
        ...
        <div class="card ">
          <div class="card-header">
            <h4 class="card-title text-dark"> La liste des Livreurs : </h4>
            ...
            <div class="table-responsive" >
              <table class="table tablesorter table-bordered" id="" style="border:2px solid gray;">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="border:2px solid gray;">{{ $livreur->nom}} {{ $livreur->prenom}} </td>
                        <td style="border:2px solid gray;">{{ $livreur->adresse}} </td>
                        <td style="border:2px solid gray;">{{ $livreur->tel}} </td>
                        <td style="border:2px solid gray;">{{$livreur->name }} </td>
                         {{--  <td style="border:2px solid gray;">
                            <form action="{{url('admin/livreurs/'.$livreur->id)}}" onsubmit="return confirm('Voulez vous vraiment supprimer ce livreur ?');" method="post">
  ...
  --}}
                       ...
                       @endforeach
                         <div class="float-right">{{ $livreurs->links() }}</div>
